    ...
    <AuthenticatedLayout>
        ...
        <div class="py-8">
            ...
            <div v-if="rentals.length > 0" class="mt-16 grid gap-x-4 gap-y-4 lg:grid-cols-2 xl:grid-cols-3">
                <div v-for="rental in rentals" :key="rental.id" class="rental-card space-y-2">
                    <div v-if="rental.images.length > 0">
                        ...
                        <RouterLink :to="{ name: 'editRentals', params: { id: rental.id } }"
                            v-if="rental.images.length > 1" class="mt-2 text-blue-500 hover:underline">
                    ...
                    <RouterLink :to="{ name: 'editRentals', params: { id: rental.id } }">
                        <p
                            class="mt-6 w-full bg-green-700 text-white py-2 rounded-lg hover:bg-green-800 transition text-center">
                            Edit Info</p>

                    </RouterLink>
                </div>
            </div>
            <div v-if="rentals.length < 1" class="mx-auto flex flex-col items-center mt-10 space-y-4">
                <h2 class="text-3xl font-bold text-center">No Rentals Posted.</h2>
                <RouterLink :to="{ name: 'rentPost' }"
                    class="bg-green-700 px-4 py-2 text-white font-bold rounded-lg text-center mx-auto">List
                    Your
                    Property</RouterLink>
            </div>
        </div>
